...
sed -i "s|\$(VERSION_DIST_SANITIZED)-|\$(VERSION_DIST_SANITIZED)-$(TZ=Asia/Shanghai date +%Y%m%d)-pw-|" include/image.mk

sed -i '/sed -i "\/log-facility\/d" "\/etc\/dnsmasq.conf"/i sed -i '\''/BUILD_ID/d'\'' /etc/os-release\necho "BUILD_ID='\''R260618'\''" >> /etc/os-release' package/emortal/default-settings/files/99-default-settings
